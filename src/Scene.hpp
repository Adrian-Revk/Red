...
namespace red {
	...
	/// Each 'object' is stored in continuous space as a :
	...
	public:
		...
		u32 AddEntity( Mesh* pMesh, Shader* pShader, glm::mat4 pMatrix );
	...
	private:
		...
		std::vector<Mesh*>			mMeshes;			///< Continuous array of Meshes that have to be rendered
		std::vector<Shader*>		mShaders;			///< Continuous array of Shaders that are used to render the meshes
		std::vector<glm::mat4>		mWorldMatrices;		///< Continuous array of World matrices that are used to move the meshes
